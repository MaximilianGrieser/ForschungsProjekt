...
class BlackBox:
    ...
    def loadData(self, database=None):
        if database is None:
            ...
            random_1 = np.random.Generator.integers(0, 12)*2
            random_2 = (np.random.Generator.integers(0, 12)*2)+1
            counter = 0
            error = 0
            cache_data = []
            cache_labels = []
            if self._isCached(database):
                ...
                if database.split("_")[0] == "actor" and (database.split("_")[1] == random_1 or database.split("_")[1] == random_2):
    ...
    def predict(self, f=None):
        ...
        if self.trained:
            if f is not None:
                ...
                self.predictions_2 = self.model.predict(self.x_test2)
                self.predictions_prob_2 = self.model.predict_proba(self.x_test2)
    ...
    def _evaluatePrediction(self):
        ...
        self.accuracy_topf = np.zeros((2, 7, 3))
        for i in range(0, len(prediction)):

            if y_test_emotion[i] == prediction[i]:
                result = "TRUE"
                self.accuracy_topf[y_test_gender[i] -
                                   1][y_test_emotion[i]-1][0] += 1
                self.accuracy_topf[y_test_gender[i] -
                                   1][y_test_emotion[i]-1][1] += 1
            else:
                result = "FALSE"
                self.accuracy_topf[y_test_gender[i] -
                                   1][y_test_emotion[i]-1][1] += 1
            ...
            if self._prediction_details:
                print("[+] [{:5s}]\nprediction={} key={}\nprob={}\n\t   {}\n---\n".format(result,
                                                                                          ...
                                                                                          emotion))
        for gender in self.accuracy_topf:
